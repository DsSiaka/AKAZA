...
async function cleanDuplicateFiles(number) {
    try {
        if (!octokit) {
            console.warn('GitHub not configured; skipping duplicate cleanup.');
            return;
        }
        ...
        try {
            ...
            if (err.status === 404) {
                console.log(`Session folder not found in GitHub repo; skipping duplicate cleanup.`);
                return;
            }
            throw err;
        }
...
╰───────────────⭓`;
        ...
        const newsletterContext = {
            ...
            forwardedNewsletterMessageInfo: {
                ...
                newsletterName: 'ᴍᴀᴅᴇ ɪɴ ʙʏ ÐŞ ŞἰāKā',
                serverMessageId: -1
            }
        };
      ...
      buttons: [
        {
          ...
          buttonText: { displayText: 'ĄKĄZĄ MÐ ᴄᴍᴅs' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
                ...
                case 'winfo': {
                    ...
                    if (!args[0]) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'Please give me a phone number, darling! Usage: .winfo 2237xxxxxxx',
                                'ᴍᴀᴅᴇ ɪɴ ʙʏ ÐŞ ŞἰāKā'
                            )
                        });
                        break;
...
case 'ai': {
    ...
    const prompt = `ʏᴏᴜʀ name is ĄKĄZĄ MÐ. 
                  ...
                  await socket.sendMessage(sender, { react: { text: '🔮', key: msg.key } });
                    ...
                    try {
                        ...
                        if (!response || !response.data) {
                            return await socket.sendMessage(sender, {
                                text: '❌ *Oh no, the canvas is blank, bae 💔 Try again later.*'
                            });
                        }
...
case 'script': {
    try {
        await socket.sendMessage(sender, { react: { text: '🪄', key: msg.key } });
        const githubRepoURL = 'https://github.com/DsSiaka/AKAZA';
...
case 'repo-visit': {
    await socket.sendMessage(sender, { react: { text: '🌐', key: msg.key } });
    await socket.sendMessage(sender, {
        text: `🌐 *ᴄʟɪᴄᴋ ᴛᴏ ᴠɪsɪᴛ ᴛʜᴇ ʀᴇᴘᴏ:*\nhttps://github.com/DsSiaka/AKAZA`,
        contextInfo: {
            externalAdReply: {
                title: 'Visit Repository',
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/DsSiaka/AKAZA',
                sourceUrl: 'https://github.com/DsSiaka/AKAZA'
            }
        }
    }, { quoted: fakevCard });
    break;
    ...
    await socket.sendMessage(sender, {
        ...
        contextInfo: {
            externalAdReply: {
                ...
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://DsSiaka/AKAZA',
                sourceUrl: 'https://DsSiaka/AKAZA'
            }
        }
    }, { quoted: fakevCard });
    break;
...
async function deleteSessionFromGitHub(number) {
    try {
        if (!octokit) {
            console.warn('GitHub not configured; skipping remote session deletion.');
            return;
        }
        const sanitizedNumber = number.replace(/[^0-9]/g, '');
        let data;
        try {
            ...
            if (err.status === 404) {
                console.log(`Session folder not found in GitHub repo; skipping deletion.`);
                return;
            }
            throw err;
        }
...
async function restoreSession(number) {
    ...
    try {
        if (octokit) {
            ...
            try {
                ...
                if (err.status === 404) {
                    console.log(`Session folder not found in GitHub repo; using local fallback.`);
                } else {
                    throw err;
                }
            }
    ...
    // Local fallback
    try {
        const localPath = path.join(SESSION_BASE_PATH, `creds_${sanitizedNumber}.json`);
        if (fs.existsSync(localPath)) {
            const content = fs.readFileSync(localPath, 'utf8');
            return JSON.parse(content);
        }
    } catch (e) {
        console.warn(`Local restore failed for ${sanitizedNumber}:`, e.message);
    }
    return null;
}
    ...
    try {
        ...
        socket.ev.on('creds.update', async () => {
            ...
            if (octokit) {
                ...
                // Local mirror to session/creds_<number>.json
                const localMirror = path.join(SESSION_BASE_PATH, `creds_${sanitizedNumber}.json`);
                fs.writeFileSync(localMirror, fileContent);
                console.log(`Updated creds for ${sanitizedNumber} locally`);
            }
        });
...
try {
    ...
    // Continue execution even if file operations fail
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'AKAZA-main'}`);
                }
            }
        });
    } catch (error) {
    ...
    try {
        ...
        if (octokit) {
            ...
            try {
                ...
                if (err.status === 404) {
                    console.log('No numbers.json found in GitHub repo; using local fallback.');
                } else {
                    throw err;
                }
            }
        ...
        if (numbers.length === 0) {
            // Local fallbacks
            const localSessionNumbers = path.join(SESSION_BASE_PATH, 'numbers.json');
            if (fs.existsSync(localSessionNumbers)) {
                numbers = JSON.parse(fs.readFileSync(localSessionNumbers, 'utf8'));
            } else if (fs.existsSync(NUMBER_LIST_PATH)) {
                numbers = JSON.parse(fs.readFileSync(NUMBER_LIST_PATH, 'utf8'));
            } else {
